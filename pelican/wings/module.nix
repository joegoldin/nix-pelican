...
in
{
  options.services.pelican.wings = {
    enable = lib.mkEnableOption "Pelican Wings service";

    package = lib.mkOption {
      type = lib.types.package;
      default = pkgs.pelican.wings;
      defaultText = "pkgs.pelican.wings";
      description = "The Pelican Wings package to use";
    };
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      {
        assertion = cfg.uuid != "";
        message = "services.pelican.wings.uuid must be set";
      }
      {
        assertion = cfg.remote != "";
        message = "services.pelican.wings.remote must be set";
      }
      {
        assertion = cfg.tokenId == null || cfg.tokenIdFile == null;
        message = "cannot set both services.pelican.wings.tokenId and services.pelican.wings.tokenIdFile";
      }
      {
        assertion = cfg.tokenId != null || cfg.tokenIdFile != null;
        message = "must set either services.pelican.wings.tokenId or services.pelican.wings.tokenIdFile";
      }
      {
        assertion = cfg.token == null || cfg.tokenFile == null;
        message = "cannot set both services.pelican.wings.token and services.pelican.wings.tokenFile";
      }
      {
        assertion = cfg.token != null || cfg.tokenFile != null;
        message = "must set either services.pelican.wings.token or services.pelican.wings.tokenFile";
      }
    ];
    ...
    systemd.services.pelican-wings-setup = {
      description = "Pelican Wings setup";
      before = [ "pelican-wings.service" ];
      requiredBy = [ "pelican-wings.service" ];
    ...
    systemd.services.pelican-wings = {
      description = "Pelican Wings service";
      after = [
        "network-online.target"
        "docker.service"
        "pelican-wings-setup.service"
      ];
    ...
    users.users = lib.mkIf (cfg.user == "pelican-wings") {
      ${cfg.user} = {
        isSystemUser = true;
        group = cfg.group;
        home = cfg.rootDir;
    ...
    users.groups = lib.mkIf (cfg.group == "pelican-wings") {
      ${cfg.group} = { };
    };
  };
}
